...
internal abstract class BaseRedisCacheService<TEntity> : ICacheService<TEntity> where TEntity : class
{
        ...
        bool @override = true)
    {
        ...
        if (!@override)
        {
            bool exists = await _redisDatabase.ExistsAsync(key);
            if (exists)
            {
                return;
            }
    ...
    public async Task SetRecordsAsync(Dictionary<string, TEntity> data, TimeSpan? absoluteExpireTime = null)
    {
        ...
        KeyValuePair<string, TEntity>[] dataArray = data.ToArray();
        Tuple<string, TEntity>[] allData = new Tuple<string, TEntity>[data.Count];
